...
#
package Optikus::Forms::Widget::Control::Offnominal;
...
my $offnominals_file = "$pforms_home/etc/offnominals.lst";
...
{
  my ($button, $self) = @_;
  if ($Optikus::Forms::Widget::SIMULATION) {
  ...
  $file =~ s/\s+/_/g;
  my $state = $on ? "ON" : "OFF";
  $file = "OFFNOMINAL-$file-$state";
  olog "offnominal $state: $self->{name}";
  Optikus::Watch::runFormScript($file);
}
