...
#
package Optikus::Forms::Widget::Meter;

use strict;
use Gtk2;
use Optikus::Forms::Util;
use Optikus::Forms::Widget::Meter::BigTank;
use Optikus::Forms::Widget::Meter::SmallTank;
use Optikus::Forms::Widget::Meter::RoundTank;
use Optikus::Forms::Widget::Meter::Volume;
use Optikus::Forms::Widget::Meter::ThermoMeter;
...
sub new
{
  ...
  my $color = $arg{color};
  $self->{GE} = create_pic("png/${subclass}_empty.png");
  $self->{GF} = create_pic("png/${subclass}_${color}.png");
  $self->{color} = $arg{color};
  my ($w, $h) = ($self->{GE}->get_width, $self->{GE}->get_height);
  $self->{WIDGET} = Gtk2::Image->new;
  ($self->{PIXMAP}, $self->{BITMASK}) = $self->{GE}->render_pixmap_and_mask(1);
...
sub redraw_pixmap
{
  ...
  $y = make_test_value(0, $self->{H}) if $Optikus::Forms::Widget::TEST;
